...
def eval_model(
    ...
    for weight in weights_list:
        ...
        print(
            f"corpus BLEU-{len(list(weight))} score with tf:",
            corpus_bleu_score_previous,
        )
...
if __name__ == "__main__":
    ...
    if test_input == "EEG" and train_input == "EEG":
        ...
        output_all_results_path = (
            f"./results/{task_name}-{model_name}-all_decoding_results.txt"
        )
        ...
        score_results = (
            f"./score_results/{task_name}-{model_name}-{train_input}_{test_input}.txt"
        )
    ...
    if model_name == "BrainTranslator":
        ...
        pretrained = BertGenerationDecoder.from_pretrained(
            "google-bert/bert-large-uncased", is_decoder=True
        )
        model = BrainTranslator(
            pretrained,
            in_feature=105 * len(bands_choice),
