...
def get_input_sample(
    ...
    if add_CLS_token:
        input_sample["input_attn_mask_invert"][: len(sent_obj["word"]) + 1] = (
            torch.zeros(len(sent_obj["word"]) + 1)
        )  # 0 is not masked
    else:
...
class SST_tenary_dataset(Dataset):
    def __init__(self, ternary_labels_dict, tokenizer, max_len=56, balance_class=True):
        ...
        if balance_class:
            print(
                f"balance class to {min([len(pos_samples), len(neg_samples), len(neu_samples)])} each..."
            )
