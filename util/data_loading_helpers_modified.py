...
Alpha_features = (
    Alpha_ffd_names + Alpha_gd_names + Alpha_gpt_names + Alpha_trt_names
)  # + Alpha_sfd_names
Beta_features = (
    Beta_ffd_names + Beta_gd_names + Beta_gpt_names + Beta_trt_names
)  # + Beta_sfd_names
Gamma_features = (
    Gamma_ffd_names + Gamma_gd_names + Gamma_gpt_names + Gamma_trt_names
)  # + Gamma_sfd_names
Theta_features = (
    Theta_ffd_names + Theta_gd_names + Theta_gpt_names + Theta_trt_names
)  # + Theta_sfd_names
...
def extract_all_fixations(
    data_container, word_data_object, float_resolution=np.float16
):
    """
    ...
    if len(word_data.shape) > 1:
        for fixation_idx in range(word_data.shape[0]):
            fixations_data.append(
                np.array(data_container[word_data[fixation_idx][0]]).astype(
                    float_resolution
                )
            )
...
def extract_word_level_data(
    data_container, word_objects, eeg_float_resolution=np.float16
):
    """
    ...
    if isinstance(available_objects[0], str):
        ...
        if "rawEEG" in available_objects:
            ...
            ####
            assert len(contentData) == len(etData) == len(rawData), (
                "different amounts of different data!!"
            )
            ...
            for (
                ...
                if is_real_word(word_string):
                    data_dict = {}
                    data_dict["RAW_EEG"] = extract_all_fixations(
                        data_container, raw_eegs_obj[0], eeg_float_resolution
                    )
                    data_dict["RAW_ET"] = extract_all_fixations(
                        data_container, ets_obj[0], np.float32
                    )

                    data_dict["FFD"] = (
                        data_container[ffd[0]][()][0, 0]
                        if len(data_container[ffd[0]][()].shape) == 2
                        else None
                    )
                    data_dict["GD"] = (
                        data_container[gd[0]][()][0, 0]
                        if len(data_container[gd[0]][()].shape) == 2
                        else None
                    )
                    data_dict["GPT"] = (
                        data_container[gpt[0]][()][0, 0]
                        if len(data_container[gpt[0]][()].shape) == 2
                        else None
                    )
                    data_dict["TRT"] = (
                        data_container[trt[0]][()][0, 0]
                        if len(data_container[trt[0]][()].shape) == 2
                        else None
                    )
                    data_dict["SFD"] = (
                        data_container[sfd[0]][()][0, 0]
                        if len(data_container[sfd[0]][()].shape) == 2
                        else None
                    )
                    data_dict["nFix"] = (
                        data_container[nFix[0]][()][0, 0]
                        if len(data_container[nFix[0]][()].shape) == 2
                        else None
                    )
                    ...
                    data_dict["ALPHA_EEG"] = np.concatenate(
                        [
                            data_container[obj[word_idx][0]][()]
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in Alpha_features_data
                        ],
                        0,
                    )

                    data_dict["BETA_EEG"] = np.concatenate(
                        [
                            data_container[obj[word_idx][0]][()]
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in Beta_features_data
                        ],
                        0,
                    )

                    data_dict["GAMMA_EEG"] = np.concatenate(
                        [
                            data_container[obj[word_idx][0]][()]
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in Gamma_features_data
                        ],
                        0,
                    )

                    data_dict["THETA_EEG"] = np.concatenate(
                        [
                            data_container[obj[word_idx][0]][()]
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in Theta_features_data
                        ],
                        0,
                    )
                    ...
                    if data_dict["nFix"] is not None:
                        ####################################
                        data_dict["GD_EEG"] = [
                            np.squeeze(data_container[obj[word_idx][0]][()])
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in GD_EEG_features
                        ]
                        data_dict["FFD_EEG"] = [
                            np.squeeze(data_container[obj[word_idx][0]][()])
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in FFD_EEG_features
                        ]
                        data_dict["TRT_EEG"] = [
                            np.squeeze(data_container[obj[word_idx][0]][()])
                            if len(data_container[obj[word_idx][0]][()].shape) == 2
                            else []
                            for obj in TRT_EEG_features
                        ]
                        ####################################
            ...
            sentence = " ".join(
                [
                    load_matlab_string(data_container[word_obj[0]])
                    for word_obj in word_objects["content"]
                ]
            )
