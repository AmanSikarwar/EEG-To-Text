...
from model_sentiment import (
    BaselineLSTM,
    BaselineMLPSentence,
    FineTunePretrainedTwoStep,
    JointBrainTranslatorSentimentClassifier,
    ZeroShotSentimentDiscovery,
)
...
def eval_model(
        ...
        for phase in ["test"]:
            ...
            for (
                ...
                if isinstance(model, BaselineMLPSentence):
                    ...
                    or isinstance(model, BartForSequenceClassification)
                ):
                    output = model(
                    ...
                    LM_output, classification_output = model(
                        input_word_eeg_features,
                        input_masks,
                        input_mask_invert,
                        target_ids,
                    ...
                    print(
                        "pred string:",
                        logits2PredString(LM_logits, tokenizer)
                        .split("</s></s>")[0]
                        .replace("<s>", ""),
                    )
    ...
    print(
        precision_recall_fscore_support(
            total_true_labels, total_pred_labels, average="micro"
        )
    )
    print()
...
if __name__ == "__main__":
    ...
    if model_name == "BaselineMLP":
        ...
        print(
            f"[INFO]Model: ZeroShotSentimentDiscovery, using classifer:{classifier_name}, using generator: {decoder_name}"
        )
        pretrained = BartForConditionalGeneration.from_pretrained("facebook/bart-large")
        if decoder_name == "BrainTranslator":
            decoder = BrainTranslator(
                pretrained,
        ...
        model = ZeroShotSentimentDiscovery(
            decoder, classifier, decoder_tokenizer, sentiment_tokenizer, device=device
        )
        model.to(device)
